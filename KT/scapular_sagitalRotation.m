...
% Define (empty) variables for further use
romPre50 = [];
romPre100 = [];
romPost50 = [];
romPost100 = [];
...
for k = 2: numFiles
    ...
    % Asign Pre Post
    rotationPre = target.Escapular_Pre_HumeroScapular; % target movement
    rotationPost = target.Escapular_Post_HumeroScapular; % target movement
    ...
    for b = 1 : 5
        ...
        [startPre50, index1] = max(A(1:round(length(A)/2),1));
        [startPost50, index2] = max(B(1:round(length(B)/2),1));
        [endPre50,index3] = min(A);
        [endPost50,index4] = min(B);

        startPre100 = max(C(1:round(length(C)/2),1));
        startPost100 = max(D(1:round(length(D)/2),1));
        endPre100 = min(C);
        endPost100 = min(D);
...
title('ROM Humerus (Sagital Rotation) - 50°')
...
plot(romPost100,'-ro')
legend({'Pre','Post'},'Location','southwest')
title('ROM Humerus (sagital Rotation) - 100°')
