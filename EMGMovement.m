% Written by Edgardo Silva, 26/06/2022
...
load('C:\Users\VICON\Documents\GitHub\ViconMatlab\data\S03.mat')

% Set up variables (cells) since data size (due frames) is no equal in data
% will be stored for both planes and four conditions (50, 100, pre, post)
frontalMovPre50 = {};
frontalMovPost50 = {};
frontalMovPre100 = {};
frontalMovPost100 = {};
...
% filter stage for Movement data, cut off frequency obtained from spectrum
% analysis
...
df = designfilt('lowpassfir','FilterOrder',100,'CutoffFrequency',Fcut); % design filter
Frontal_Pre_ScapularTilt = filtfilt(df,Frontal_Pre_ScapularTilt); % Frontal plane - Pre
Frontal_Post_ScapularTilt = filtfilt(df,Frontal_Post_ScapularTilt); % Frontal plane - Post

df = designfilt('lowpassfir','FilterOrder',100,'CutoffFrequency',Fcut); % design filter
Frontal_Pre_ScapularUpward = filtfilt(df,Frontal_Pre_ScapularUpward); % Frontal plane - Pre
Frontal_Post_ScapularUpward = filtfilt(df,Frontal_Post_ScapularUpward); % Frontal plane - Post

df = designfilt('lowpassfir','FilterOrder',100,'CutoffFrequency',Fcut); % design filter
Frontal_Pre_HumeroFrontal = filtfilt(df,Frontal_Pre_HumeroFrontal); % Frontal plane - Pre
Frontal_Post_HumeroFrontal = filtfilt(df,Frontal_Post_HumeroFrontal); % Frontal plane - Post

df = designfilt('lowpassfir','FilterOrder',100,'CutoffFrequency',Fcut); % design filter
Escapular_Pre_HumeroScapular = filtfilt(df,Escapular_Pre_HumeroScapular); % Escapular plane - Pre
Escapular_Post_HumeroScapular = filtfilt(df,Escapular_Post_HumeroScapular); % Escapular plane - Post
% End of filter stage

% Normalization of sEMG
% Since we captured MVC for volunteers before each experimental session,
% sEMG captured from dynamic trials has to be normalized to determine the
% percentage of activation of the target muscle regarding its Maximum
% value.

target1 = serratoMVC; % muscle 1. Serratus Anterior
target2 = trapezioMVC; % muscle 2. Lower Trapezius

% we have three (3) trials of MVC for each participant, then we need to
% devide our data into three sections, using the following:
target1 = reshape(target1,[],3);
target2 = reshape(target2,[],3);

peakRmsSe = []; % emtpy variable for storage
peakRmsTr = []; % emtpy variable for storage

% we will calculate the rms for each section of MVC captured
for i = 1: 3
    peakRmsSe = [peakRmsSe, rms(target1(:,i))];
    peakRmsTr = [peakRmsTr, rms(target2(:,i))];
end

peakRmsSe = max(peakRmsSe); % store max value of rms
peakRmsTr = max(peakRmsTr); % store max value of rms

% This section will focused solely into sorting data, since we have two
% planes (Frontal and Escapular/Sagital), two conditions (Pre and Post) and two target
% angles (50 and 100), we will use information from kinematic data, in
% which movement is detected from frame-by-frame observation.
% Using those frames, we will cut our data into columns stored into cells
% (as previously mentioned, data size is different for each column. Hence
% the use of cells
for i=1:10
    if Trials(i) == 50
        % Movement
        ...
        % Muscles (*20 due differences in sampling rate -> EMG was captured
        % at 2000Hz)
        ...
        sagitalMusclePre50 = [sagitalMusclePre50, sagital_serratoPre50_ROI, sagital_trapezioPre50_ROI]; % Serrato - Trapezio Pre 50 (Escapular plane)
        sagitalMusclePost50 = [sagitalMusclePost50, sagital_serratoPost50_ROI, sagital_trapezioPost50_ROI]; % Serrato - Trapezio Post 50 (Escapular plane)
        ...
        sagitalMovPre50 = [sagitalMovPre50, data4Pre50]; % HumeroAbd Pre 50 (Escapular plane)
        sagitalMovPost50 = [sagitalMovPost50, data4Post50]; % HumeroAbd Post 50 (Escapular plane)
    ...
    elseif Trials(i) == 100
        ...
        sagitalMusclePre100 = [sagitalMusclePre100, sagital_serratoPre100_ROI, sagital_trapezioPre100_ROI]; % Serrato - Trapezio Pre 100 (Escapular plane)
        sagitalMusclePost100 = [sagitalMusclePost100, sagital_serratoPost100_ROI, sagital_trapezioPost100_ROI]; % Serrato - Trapezio Pre 100 (Escapular plane)
        ...
        sagitalMovPre100 = [sagitalMovPre100, data4Pre100]; % HumeroAbd Pre 100 (Escapular plane)
        sagitalMovPost100 = [sagitalMovPost100, data4Post100]; % HumeroAbd Post 100 (Escapular plane)
    end

end

% lets clear the workspace a litle bit...
clearvars -except...
    sagitalMusclePre50 sagitalMusclePost50...
    sagitalMusclePre100 sagitalMusclePost100 frontalMusclePre50...
    frontalMusclePost50 frontalMusclePre100 frontalMusclePost100...
    sagitalMusclePost50 sagitalMusclePre100 sagitalMusclePost100...
    frontalMovPre50 frontalMovPost50 frontalMovPre100 frontalMovPost100...
    sagitalMovPre50 sagitalMovPost50 sagitalMovPre100 sagitalMovPost100...
    peakRmsTr peakRmsSe

%{
data is distributed according to: frontalMov = {ScapularTilt,
ScapularUpward, HumeroFrontal, ScapularTilt, ScapularUpward, HumeroFrontal,
ScapularTilt, ScapularUpward, HumeroFrontal, ...} sagitalMov =
{HumeroScapular, HumeroScapular, HumeroScapular, HumeroScapular, ...}
frontalMuscle = {Serrato, Trapezio, ,Serrato, Trapezio, ...} sagitalMuscle
= {Serrato, Trapezio, ,Serrato, Trapezio, ...}
%}

% Now, lets calculate some useful information. 
% First. FrontalRomCalculation takes all movements from the frontal plane
% (Scapular Tilt, Scapular Upward Rotation, and Humeral Abduction) and
% calculate its ROM according to the desired target angle (50 or 100)
[scapTiltRomPre50, scapTiltRomPost50, scapTiltRomPre100,scapTiltRomPost100,...
    scapUpRomPre50, scapUpRomPost50,scapUpRomPre100, scapUpRomPost100,...
    abdRomPre50, abdRomPost50, abdRomPre100, abdRomPost100] = ...
frontalRomCalculation(frontalMovPre50,frontalMovPost50, frontalMovPre100, frontalMovPost100);

% SagitalRomCalculation does the same as the above, only for Sagital Plane
[sagitalbAbdRomPre50, sagitalbAbdRomPost50, sagitalbAbdRomPre100,sagitalbAbdRomPost100] = ...
sagitalRomCalculation(sagitalMovPre50,sagitalMovPost50, sagitalMovPre100, sagitalMovPost100);

% Once movement is determined, here comes the EMG analysis
% As suggested from the PI, frontal and sagital rawCalculation will
% calculate the mean of raw EMG voltage data
[serratoPre50, serratoPre100, serratoPost50, serratoPost100,...
    trapezioPre50,trapezioPre100, trapezioPost50, trapezioPost100] = ...
frontal_rawCalculation(frontalMusclePre50,frontalMusclePost50,frontalMusclePre100, frontalMusclePost100);
...
sagital_rawCalculation(sagitalMusclePre50,sagitalMusclePost50, sagitalMusclePre100, sagitalMusclePost100);

% as for the following, both functions will take muscular activity from
% both target muscles and calculate its RMS, then, normlization will be
% taken place in order to show each muscle regarding their maximum
% activity.
[rmsSerratoPre50, rmsSerratoPre100, rmsSerratoPost50,rmsSerratoPost100,...
    rmsTrapezioPre50, rmsTrapezioPre100, rmsTrapezioPost50, rmsTrapezioPost100] = ...
frontal_rmsCalculation(frontalMusclePre50,frontalMusclePost50, frontalMusclePre100, frontalMusclePost100,peakRmsSe,peakRmsTr);

[sagitalRmsSerratoPre50, sagitalRmsSerratoPre100, sagitalRmsSerratoPost50,sagitalRmsSerratoPost100,...
    sagitalRmsTrapezioPre50,sagitalRmsTrapezioPre100, sagitalRmsTrapezioPost50,sagitalRmsTrapezioPost100] = ...
sagital_rmsCalculation(sagitalMusclePre50,sagitalMusclePost50, sagitalMusclePre100, sagitalMusclePost100, peakRmsSe,peakRmsTr);

% Use this to export data resulting from all functions into csv
%{
zero = zeros(1,5); varNames =
{'Pre50';'Post50';'Pre100';'Post100';'SPre50';'SPost50';'SPre100';'SPost100'};
F = table( ... [abdRomPre50; scapUpRomPre50;
scapTiltRomPre50;serratoPre50;trapezioPre50;rmsSerratoPre50;rmsTrapezioPre50],
... [abdRomPost50; scapUpRomPost50;
scapTiltRomPost50;serratoPost50;trapezioPost50;rmsSerratoPost50;rmsTrapezioPost50],
... [abdRomPre100; scapUpRomPre100;
scapTiltRomPre100;serratoPre100;trapezioPre100;rmsSerratoPre100;rmsTrapezioPre100],
... [abdRomPost100; scapUpRomPost100;
scapTiltRomPost100;serratoPost100;trapezioPost100;rmsSerratoPost100;rmsTrapezioPost100],
... [sagitalbAbdRomPre50; zero; zero ;sagitalSerratoPre50;
sagitalTrapezioPre50;sagitalRmsSerratoPre50;sagitalRmsTrapezioPre50], ...
[sagitalbAbdRomPost50; zero ; zero ;sagitalSerratoPost50;
sagitalTrapezioPost50;sagitalRmsSerratoPost50;sagitalRmsTrapezioPost50],
... [sagitalbAbdRomPre100; zero ; zero ;sagitalSerratoPre100;
sagitalTrapezioPre100;sagitalRmsSerratoPre100;sagitalRmsTrapezioPre100],...
[sagitalbAbdRomPost100; zero ; zero ;sagitalSerratoPost100;
sagitalTrapezioPost100;sagitalRmsSerratoPost100;sagitalRmsTrapezioPost100],...
'VariableNames',varNames); writetable(F,'data.csv');
%}
%close all

disp('All done, you might check your results');
